	...
	<table>
		...
		<tbody>
						{#each cereri as r}
							...
							{@const expired = r.status === 'nou' && r.preferred_date && new Date(r.preferred_date.split('T')[0]) < new Date(new Date().toDateString())}
							<tr class:expirat={expired}>
								...
								<td>
									...
									{/if}
									{#if expired}
										<div class="expirat-badge">⏰ Expirată</div>
									{/if}
								</td>
								<td>
...
<style>
	...
	.btn-converteste:hover { background: var(--primary-tint); border-color: var(--primary); }
		...
		.expirat { background: #fff0f0; }
		.expirat:hover { background: #ffe0e0; }
		.expirat-badge { margin-top: 4px; font-size: 1.1rem; color: #e03131; font-weight: 600; }
</style>
